...
const VideoContentSection = ({ onOpenVideoCallPopup }) => {
  ...
  const videoId = 'sNM2wPwmS4E'; // Extracted from your URL
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 bg-white flex flex-col lg:flex-row items-center gap-8 sm:gap-12 lg:gap-16">
      ...
      {/* Right Side - YouTube Video with Custom Overlay */}
      <div className="w-full lg:w-1/2">
        <div className="relative w-full h-64 sm:h-80 lg:h-96 rounded-2xl overflow-hidden shadow-2xl bg-gradient-to-br from-primary to-red-600 transform hover:scale-105 transition-all duration-500">
          ...
          {/* Custom Overlay (poster + play button + text) - hidden when playing */}
          {!isPlaying && (
            <div
              className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center cursor-pointer group"
              onClick={handlePlayVideo}
            >
              {/* Background thumbnail for better preview (optional but recommended) */}
              <img
                src={`https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`}
                alt="Video thumbnail - Ratan Decor Euro Pratik Display"
